...
import VeiculoTable from "../components/veiculos/VeiculosTable";
import VeiculoEdit from "../components/veiculos/VeiculoEdit";
import VeiculoCreate from "../components/veiculos/VeiculoCreate";
...
export const Veiculos = () => {
  ...
  useEffect(() => {
    const carregarVeiculos = async () => {
      ...
      try {
        ...
        setSnackbar({ open: true, message: "Erro ao buscar veículos.", severity: "error", });
        setVeiculos([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = useCallback(async (id: number) => {
    ...
    try {
      ...
      setVeiculos((prev) => prev.filter((p) => p.id !== id));
      setSnackbar({ open: true, message: "Veículo removido ", severity: "success", });
    } catch (error) {
      setSnackbar({ open: true, message: "Erro ao deletar veículo.", severity: "error", });
    } finally {
      setDeletingId(null);
    }
  }, []);
  ...
  const handleSaveVeiculo = useCallback((veiculoAtualizado: Veiculo) => {
    setVeiculos((prev) =>
      prev.map((p) => (p.id === veiculoAtualizado.id ? veiculoAtualizado : p))
    );
    setSnackbar({ open: true, message: "Veículo atualizado com sucesso.", severity: "success", });
  }, []);
  ...
  const handleSucessoCriarVeiculo = useCallback(
    (novoVeiculo: Veiculo) => {
  ...
  const veiculosFiltrados = veiculos.filter((v) => {
    ...
    return (
      ...
      v.cliente?.cpf?.replace(/\D/g, "").includes(termo)
    );
  });
    ...
    >
      ...
      >
        ...
        <Box mb={4}>
          <TextField
            fullWidth
            placeholder="Buscar por Modelo, Ano, Chassi ou Cliente"
            value={searchTerm}
          ...
          {debouncedSearchTerm && (
            <Box mt={2} display="flex" alignItems="center" gap={1} ml={1}>
              ...
              <Chip 
                label={veiculosFiltrados.length}
                size="small" 
                sx={{
